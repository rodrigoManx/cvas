...
(function () {
	jDBSCAN = function () {
		...
		function expand_cluster(point_idx, neighbours, cluster_idx) {
			...
			data[point_idx].clusters[exploration] = cluster_idx;	//assign cluster id

			for (var i = 0; i < neighbours.length; i++) {
				var curr_point_idx = neighbours[i];
				if (data[curr_point_idx].clusters[exploration] === undefined) {
					data[curr_point_idx].clusters[exploration] = -1;
				...
				if (data[curr_point_idx].clusters[exploration] < 0) { // not assigned to a cluster but visited (= 0)
					data[curr_point_idx].clusters[exploration] = cluster_idx;
		...
		var dbscan = function () {
			...
			cluster_id = 1;
			for (var i = 0; i < data.length; i++) {
				if (data[i].clusters[exploration] == undefined) {
					//console.log(exploration);
					data[i].clusters[exploration] = -1; //visited and marked as noise by default
					var neighbours = get_region_neighbours(i);
					//console.log(neighbours);
					var num_neighbours = neighbours.length;
					if (num_neighbours < minPts) {
						data[i].clusters[exploration] = -1; //noise
					} 
					else {
						//empty new cluster
		...
		dbscan.exploration = function (expl) {
			if (arguments.length === 0) {
				return exploration;
			}
			if (typeof expl === 'number') {
				exploration = expl;
			}
